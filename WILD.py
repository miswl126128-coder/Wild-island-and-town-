...
class GameState:
    ...
    def build_menu(self):
        ...
        if self.stone < 1: missing.append(f"石头x{1-self.stone}")
        if self.wood + self.driftwood < 1: missing.append(f"木材/漂流木x{1-(self.wood+self.driftwood)}")
        show_item(4, "🔪 石刀", self.has_stone_knife, not missing, "建造（石头x1 + 木材/漂流木x1）", missing)

        # 5 鱼竿
        missing = []
        if self.wood < 2: missing.append(f"木材x{2-self.wood}")
        if self.vine < 1: missing.append(f"藤蔓x{1-self.vine}")
        show_item(5, "🎣 鱼竿", self.has_fishing_rod, not missing, "建造（木材x2 + 藤蔓x1）", missing)
...
""")
        ...
        while self.alive and not self.escaped:
            ...
            locked_cmds = {"cw","cs","cv","cwater","cflint","pick","gather","fish","hunt","build","carrywater","cook"}
