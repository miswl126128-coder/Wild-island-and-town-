...
class GameState:
    def __init__(self):
        ...
        self.vine = 0
        self.flint = 0
        self.fish_count = 0   # 修复：原fish与方法名冲突，改为fish_count
        self.fruit = 0
        self.herb = 0
        self.shells = 0
        self.hide = 0
    ...
    # 修复：改名为 go_fish，避免与属性 fish_count 混淆
    def go_fish(self):
        if self.location != 0:
            print("🎣 钓鱼需要在沙滩海边。")
            return
    ...
    def handle_storm(self):
        if self.day == 1 and self.storm_day is None:
            self.storm_day = 2 + random.randint(0, 1)
            print(f"\n💨 你感觉到海风中有一种不寻常的气息……（风暴将在第 {self.storm_day} 天夜里来临）")
        # 修复：storm_day为None时跳过比较
