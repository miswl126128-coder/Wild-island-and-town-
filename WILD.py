...
class GameState:
    def __init__(self):
        ...
        # ===== 资源 =====
        self.wood = 0          # 木材（中心砍树获得）
        self.driftwood = 0     # 漂流木（沙滩捡到）
        self.stone = 0         # 石头（中心采集）
        self.vine = 0          # 藤蔓（中部采集）
        self.flint = 0         # 燧石（需要石斧，中心采集）
        self.fish = 0          # 鱼（沙滩钓鱼）
        self.fruit = 0         # 野果（中部采集）
        self.herb = 0          # 药草（中部概率获得）
        self.shells = 0        # 贝壳（沙滩拾取）
        self.hide = 0          # 兽皮（狩猎）
        self.bone = 0          # 骨头（狩猎）
        self.coconut = 0       # 椰子壳（中心采集）
        self.water_units = 0   # 淡水储备（中心泉水采集）
        ...
        # ===== 营地物品 =====
        self.camp_water = 0     # 庇护所里储存的淡水（椰子壳装）
    ...
    def show_actions(self):
        ...
        print("\n📋 行动指令:")
        print("  【移动】")
        print("    m1  前往沙滩    m2  前往中部灌木丛    m3  前往中心树林")
        print("  【采集】")
    ...
    def gather_middle(self):
        ...
        elif roll < 0.75:  # 药草 20%
            self.herb += 1
            print(f"🌿 你找到了一株药草！（当前: {self.herb}）")
            print("   💡 药草可以治疗 +4HP，也可以作为离开荒岛时的备用医疗。）
        elif roll < 0.90:  # 藤蔓 15%
    ...
    def fish(self):
        ...
        if self.period == 0:  # 清晨
            success_rate = 0.85
        elif self.period == 3:  # 夜晚
            success_rate = 0.5
            
        if random.random() < success_rate:
            amount = random.randint(1, 2)
            self.fish += amount
            print(f"🐟 你钓到了 {amount} 条鱼！（当前: {self.fish}）")
        else:
    ...
    def build_menu(self):
        ...
        if not self.has_shelter:
            missing = []
            if self.vine < 2:
                missing.append(f"藤蔓x{2-self.vine}")
            if self.wood + self.driftwood < 5:
                missing.append(f"木材/漂流木x{5-(self.wood+self.driftwood)}")
            if missing:
                projects.append(("🏕️ 庇护所", "❌ 缺少: " + ", ".join(missing), False))
            else:
                projects.append(("🏕️ 庇护所", "▶️ [1] 建造（藤蔓x2 + 木材/漂流木x5）", True))
                
        # 火堆：燧石3 + 木材/漂流木2
        if not self.has_fire:
            missing = []
            if self.flint < 3:
                missing.append(f"燧石x{3-self.flint}")
            if self.wood + self.driftwood < 2:
                missing.append(f"木材/漂流木x{2-(self.wood+self.driftwood)}")
            if missing:
                projects.append(("🔥 火堆", "❌ 缺少: " + ", ".join(missing), False))
            else:
                projects.append(("🔥 火堆", "▶️ [2] 建造（燧石x3 + 木材/漂流木x2）", True))
                
        # 石斧：石头1 + 木材1
        if not self.has_stone_axe:
            missing = []
            if self.stone < 1:
                missing.append(f"石头x{1-self.stone}")
            if self.wood < 1:
                missing.append(f"木材x{1-self.wood}")
            if missing:
                projects.append(("🪓 石斧", "❌ 缺少: " + ", ".join(missing), False))
            else:
                projects.append(("🪓 石斧", "▶️ [3] 建造（石头x1 + 木材x1）", True))
        ...
        if not self.has_stone_knife:
            ...
            if self.wood < 1:
                missing.append(f"木材x{1-self.wood}")
            if missing:
                projects.append(("🔪 石刀", "❌ 缺少: " + ", ".join(missing), False))
            else:
                projects.append(("🔪 石刀", "▶️ [4] 建造（石头x1 + 木材x1）", True))
                
        # 鱼竿：木材2 + 藤蔓1
        if not self.has_fishing_rod:
            missing = []
            if self.wood < 2:
                missing.append(f"木材x{2-self.wood}")
            if self.vine < 1:
                missing.append(f"藤蔓x{1-self.vine}")
            if missing:
                projects.append(("🎣 鱼竿", "❌ 缺少: " + ", ".join(missing), False))
            else:
                projects.append(("🎣 鱼竿", "▶️ [5] 建造（木材x2 + 藤蔓x1）", True))
                
        # 木筏：藤蔓15 + 木材/漂流木30（需要石斧）
        if not self.raft_built:
            if not self.has_stone_axe:
                ...
                missing = []
                if self.vine < 15:
                    missing.append(f"藤蔓x{15-self.vine}")
                if self.wood + self.driftwood < 30:
                    missing.append(f"木材/漂流木x{30-(self.wood+self.driftwood)}")
                if missing:
                    projects.append(("⛵ 木筏", f"❌ 缺少: " + ", ".join(missing), False))
                else:
                    projects.append(("⛵ 木筏", f"▶️ [6] 建造（当前进度: {int(self.raft_progress/30*100)}%）", True))
        ...
        if choice == "0":
            return
        elif choice == "1" and not self.has_shelter:
            self.build_shelter()
        elif choice == "2" and not self.has_fire:
            self.build_fire()
        elif choice == "3" and not self.has_stone_axe:
            self.build_axe()
        elif choice == "4" and not self.has_stone_knife:
            self.build_knife()
        elif choice == "5" and not self.has_fishing_rod:
            self.build_rod()
        elif choice == "6" and not self.raft_built:
            self.build_raft()
        elif choice == "7" and self.raft_built and not self.raft_stored_safely:
            self.store_raft()
        else:
            print("❌ 无效选择")
    ...
    def build_raft(self):
        ...
        if self.raft_progress == 0:
            print(f"🪵 你需要收集 {needed_wood} 份木材/漂流木和 {needed_vine} 根藤蔓来建造木筏。")
            print("   💡 每次建造投入 1 份材料组合，进度+1")
        ...
        # 消耗材料（每次投入木材/漂流木1 + 藤蔓1）
        if self.wood + self.driftwood >= 1 and self.vine >= 1:
            if self.driftwood > 0:
                self.driftwood -= 1
            else:
                self.wood -= 1
    ...
    def eat(self):
        ...
        if self.fruit > 0:
            self.fruit -= 1
            self.food = min(10, self.food + 2)
            print(f"🍎 你吃了一些野果，饱食 +2（当前: {self.food}/10）")
            return
        print("🍽️ 你没有可食用的食物。")
    ...
    def handle_storm(self):
        """处理风暴逻辑"""
        # 第一天：确定风暴日（第2~3天夜里随机选一天）
        if self.day == 1 and self.storm_day is None:
            self.storm_day = 2 + random.randint(0, 1)  # 2或3
            print(f"\n💨 你感觉到海风中有一种不寻常的气息……（风暴将在第 {self.storm_day} 天夜里来临）")
            
        # 如果风暴已触发或还没到日子，跳过
        if self.storm_triggered or self.day < self.storm_day:
            return
            
        # 到达风暴日 → 触发风暴
        if self.day == self.storm_day and not self.storm_triggered:
            self.storm_triggered = True
            self.storm_active = True
            self.storm_duration = 1 + random.randint(0, 1)  # 持续1~2天
        ...
        if self.storm_active and self.day > self.storm_day:
            ...
            if self.storm_duration <= 0:
                ...
                print("\n☀️ 暴风雨终于停了……天空开始放晴。")
                if not self.has_shelter:
                    print("💡 你浑身湿透，在阳光下慢慢暖和起来。")
                if not self.raft_built and self.raft_progress < 15:
                    print("💡 你失去了大部分材料……但你还活着。")
...
""")
        ...
        while self.alive and not self.escaped:
            ...
            # 其他
                ...
                self.store_raft()
            elif action == "useherb" or action == "herb":
                self.use_herb()
            # 离开
            elif action == "leave":
                self.try_leave()
