...
class GameState:
    def __init__(self):
        ...
        self.stone = 0
        self.vine = 0
        self.fish_count = 0      # ✅ 改成了 fish_count
        self.fruit = 0
        self.water_units = 5
        self.shells = 0
        self.hide = 0
    ...
    def take_action(self, action):
        ...
        if action not in ["end_day", "move_to_beach", "move_to_middle", "move_to_center"]:
    ...
    def fish(self):
        ...
        if random.random() < success_rate:
            ...
            self.fish_count += amount      # ✅ 改成 fish_count
            self.food = min(10, self.food + 1)
            print(f"🐟 你钓到了 {amount} 条鱼！ (当前鱼获: {self.fish_count}, 食物: {self.food}/10)")
        else:
    ...
    def build_menu(self):
        ...
        if not self.has_fire:
            options.append(("1", "🔥 火堆 (木材x3 + 石头x1)", self.build_fire))
        if not self.has_shelter:
    ...
    def explore_beach(self):
        ...
        print("🏖️ 你在沙滩上漫步...")
        events = [
            ("捡到了一些漂流木", lambda: setattr(self, 'wood', self.wood + 1)),
            ("找到了一些贝壳", lambda: setattr(self, 'shells', self.shells + 1)),
    ...
    def explore_middle(self):
        ...
        print("🌿 你在灌木丛中穿行...")
        events = [
            ("发现了一些可食用的野果", lambda: setattr(self, 'fruit', self.fruit + 1)),
            ("找到了一些藤蔓", lambda: setattr(self, 'vine', self.vine + 1)),
    ...
    def show_status(self):
        ...
        print("-"*50)
        print(f"🪵 木材:{self.wood}  🪨 石头:{self.stone}  🌿 藤蔓:{self.vine}")
        print(f"💧 淡水储备:{self.water_units}  🐟 鱼获:{self.fish_count}  🍎 野果:{self.fruit}")
